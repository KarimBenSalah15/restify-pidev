...
public class ReclamationPageControlllers {
    ...
    void saveR(ActionEvent event) {
        ...
        ReclamtionCrud rc = new ReclamtionCrud();
    Reclamation r = new Reclamation(currentDate, false, idRR.getValue(), message.getText(),MyConnection.getInstance().getIdenvoi(),choise);
    rc.ajouterEntite(r);
    ...
    public void afficher() {
        ...
        List<Reclamation> filteredRep = rep.stream()
                .filter(reclamation -> reclamation.getUserId() == MyConnection.getInstance().getIdenvoi())
    ...
    void initialize() {
        ...
        List<Plat> plats=pc.afficherEntiite2();
        FenetreAffichage uc=new FenetreAffichage();
        List<Utilisateur> users=uc.afficher();
        ReclamtionCrud tt=new ReclamtionCrud();
        List<Reclamation> rr=tt.afficherEntiite();

        List<Utilisateur> employes = users.stream()
                .filter(user -> "Employé".equals(user.getRole()))
                .collect(Collectors.toList());


        List<Integer> idROList = rr.stream()
                .map(Reclamation::getPlatId)
                .collect(Collectors.toList());

        List<Utilisateur> userPlats = employes.stream()
        ...
        List<String> choices = Arrays.asList("plats");
    ...
    void updateR(ActionEvent event) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            Reclamation r = new Reclamation(dat, eta, typ, message1.getText(),client,plat);
